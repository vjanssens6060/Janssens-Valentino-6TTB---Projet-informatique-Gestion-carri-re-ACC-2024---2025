...
class Carriere:
    def __init__(self, nom_pilote, equipe, voiture_preferee):
        self.id = str(uuid.uuid4())  # Ajout d'un ID unique
    ...
    def to_dict(self):
        return {
            ...
            "courses": [course.to_dict() for course in self.courses],
            "reglages": [reglage.to_dict() for reglage in self.reglages],
            "historique": self.historique.to_dict()
        }
    ...
    def from_dict(cls, data):
        ...
        # Chargement des courses et réglages
        for course_data in data.get("courses", []):
            course = Course.from_dict(course_data)
            carriere.courses.append(course)
        ...
        carriere.historique = HistoriqueCarriere.from_dict(data.get("historique", {}))
        
        return carriere

    @staticmethod
    def charger_carrieres():
        """Charge toutes les carrières depuis le fichier JSON."""
        try:
            with open('data/carrieres_data.json', 'r') as f:
                data = json.load(f)
                carrieres = []
                for carriere_data in data.get("carrieres", []):
                    carriere = Carriere.from_dict(carriere_data)
    ...
    @staticmethod
    def enregistrer_carrieres(carrieres, carriere_active=None):
        """Enregistre toutes les carrières dans le fichier JSON."""
        data = {
