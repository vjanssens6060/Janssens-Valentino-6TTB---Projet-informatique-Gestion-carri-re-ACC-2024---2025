...
class Application(ctk.CTk):
    ...
    def changer_carriere(self, selection):
        ...
        if nouvelle_carriere:
            self.carriere = nouvelle_carriere
            self.mettre_a_jour_interface_carriere()
            print(f"✅ Changement vers la carrière de {nouvelle_carriere.nom_pilote}")
        else:
            print("⚠️ Erreur lors du changement de carrière")
    ...
    def ajouter_objectif(self):
        """Ajoute un nouvel objectif à la carrière."""
    ...
    def afficher_objectifs(self):
        """Affiche les objectifs dans la listbox."""
        if not hasattr(self, 'objectifs_listbox'):
            return
        ...
        for obj in self.carriere.objectifs:
            ...
            self.objectifs_listbox.insert(tk.END, 
                f"{status_emoji} [{obj.saison}] {obj.description} - Objectif: {obj.valeur_cible} {obj.type_objectif}")
