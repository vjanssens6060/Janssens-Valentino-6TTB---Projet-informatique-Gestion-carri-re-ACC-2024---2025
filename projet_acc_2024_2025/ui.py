...
class Application(ctk.CTk):
    ...
    def afficher_statistiques(self):
        ...
        if not self.carriere:
            self.stats_textbox.configure(state="normal")  # Changé de config à configure
            self.stats_textbox.delete("1.0", tk.END)
            self.stats_textbox.insert(tk.END, "Aucune carrière chargée.")
            self.stats_textbox.configure(state="disabled")  # Changé de config à configure
            return
        ...
        # Convertir classement en entier pour les calculs
        ...
        self.stats_textbox.insert(tk.END, f"Abandons : {abandons}\n")

        # Meilleur temps au tour par circuit
        meilleur_temps_par_circuit = {}
        for course in self.carriere.courses:
            circuit = course.circuit
            temps = self._convertir_temps(course.meilleur_temps)
            if circuit not in meilleur_temps_par_circuit or (temps is not None and temps < meilleur_temps_par_circuit[circuit]):
                meilleur_temps_par_circuit[circuit] = temps

        self.stats_textbox.insert(tk.END, "Meilleur temps au tour par circuit :\n")
        for circuit, temps in meilleur_temps_par_circuit.items():
            self.stats_textbox.insert(tk.END, f"  - {circuit} : {temps:.2f} secondes\n" if temps is not None else f"  - {circuit} : Non spécifié\n")
